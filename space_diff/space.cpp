...
namespace argos {
   ...
   class RingGenerator : public RealNumberGenerator {
   public:
      RingGenerator(const CVector3 c_center,
                    const UInt32 unRingQuantity,
                    const Real fZValue,
                    const Real fSegment) :
         m_cCenter(c_center),
         m_unRingQuantity(unRingQuantity),
         m_fZValue(fZValue),
         m_fSegment(fSegment) {}
      inline virtual CVector3 operator()(bool b_is_retry) {
         Real fRandK = CSimulator::GetInstance().GetRNG()->Uniform(CRange<Real>(0.0, 1.0));
         fRandK = fRandK * 2*3.14159265358979323846;
         Real fRad = CSimulator::GetInstance().GetRNG()->Uniform(CRange<Real>(0.0, 1.0));
         Real fRadius = m_fSegment * fRad + 2.5;
         Real fRandX = fRadius * cos (fRandK);
         Real fRandY = fRadius * sin (fRandK);
         return CVector3(m_cCenter.GetX() + fRandX, m_cCenter.GetY() + fRandY, m_cCenter.GetZ() + m_fZValue);
      }
   private:
      CVector3 m_cCenter;
      UInt32 m_unRingQuantity;
      Real m_fZValue;
      Real m_fSegment;
   };
   ...
   RealNumberGenerator* CreateGenerator(TConfigurationNode& t_tree) {
      ...
      else if(strMethod == "ring") {
         CVector3 cCenter;
         UInt32 unRingQuantity;
         Real fZValue;
         Real fSegment;
         GetNodeAttribute(t_tree, "center", cCenter);
         GetNodeAttribute(t_tree, "quantity", unRingQuantity);
         GetNodeAttribute(t_tree, "z_value", fZValue);
         GetNodeAttribute(t_tree, "outerwall_radius", fSegment);
         return new RingGenerator(cCenter, unRingQuantity, fZValue, fSegment);
      }
      else {
